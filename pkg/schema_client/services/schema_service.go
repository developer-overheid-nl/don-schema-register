...
import (
	...
	typesense "github.com/developer-overheid-nl/don-schema-register/pkg/schema_client/services/typesense"
)
...
func (s *SchemaService) publishToTypesense(schema models.Schema) {
	...
	if err := typesense.PublishSchema(ctx, &schema); err != nil {
...
func (s *SchemaService) PublishAllSchemasToTypesense(ctx context.Context) error {
	...
	for _, schema := range allSchemas {
		...
		err := typesense.PublishSchema(itemCtx, &schemaCopy)
		cancel()
		if err != nil {
			if errors.Is(err, typesense.ErrDisabled) {
