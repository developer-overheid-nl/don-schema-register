...
import (
	...
	"github.com/developer-overheid-nl/don-schema-register/pkg/schema_client/services/typesense"
)

func TestPublishSchema_Disabled(t *testing.T) {
	t.Setenv("TYPESENSE_ENDPOINT", "")
	t.Setenv("TYPESENSE_API_KEY", "")
	t.Setenv("TYPESENSE_COLLECTION", "")

	err := typesense.PublishSchema(context.Background(), &models.Schema{Id: "schema-1"})
	if !errors.Is(err, typesense.ErrDisabled) {
		t.Fatalf("expected ErrDisabled, got %v", err)
	}
}

func TestPublishSchema_FeatureFlagDisabled(t *testing.T) {
...
func TestPublishSchema_SendsDocument(t *testing.T) {
	...
	if err := typesense.PublishSchema(context.Background(), schema); err != nil {
...
func TestPublishSchema_ErrorStatus(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
